...
class UserRegistrationSerializer extends Serializer {
  val SEPARATOR = "//"
  override def identifier: Int = 53278 // any number works
...
object CustomSerialization extends App {
  ...
  /*
    How serialization works:
      ...
      actor calls `persist` method
      serializer serializes the event into bytes
      the journal writes the bytes

    How deserialization works:
      actor reads from the journal the bytes
      serializer deserializes the event into bytes
      actor calls `receiveRecover` method
   */
...
/*  for (i <- 1 to 10) {
    userRegistrationActor ! RegisterUser(s"user_$i@javt.com", s"User $i")
  }*/
